...
class EditorPlayState extends MusicBeatSubstate
{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (floorSus > 0)
				{
					for (susNote in 0...floorSus + 1)
					{
						...
						else if ((ClientPrefs.data.middleScroll || PlayState.doubleChart))
						{
							sustainNote.x += 310;
	...
	{
		var strumLineX:Float = (ClientPrefs.data.middleScroll || PlayState.doubleChart) ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X;
		...
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				if (!ClientPrefs.data.opponentStrums || PlayState.doubleChart)
					targetAlpha = 0;
